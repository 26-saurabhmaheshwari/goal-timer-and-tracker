...
- `sec` 4-tier: `'active'` = **Live Nuggets** (cap 5, the ONLY tier in planning/pace math — every math site tests `sec==='active'`), `'progress'` = **Quarterly** (cap 11), `'yearly'` = **Yearly** (cap 21), anything else = **Someday** (legacy backlog/dream fall through). Caps hard-block in goalForm submit; minis exempt. Per-category nugget quota `LS_QUOTA` = soft-warn only. Logged time ALWAYS counts toward totals regardless of tier.
...
- **Minis** (`parentId` set, one level only): progress echoes to parent (`cur+=amt` + `{via:name}` log row, NO second time entry). Log lists → `loggableGoals()` (parent with live active minis is replaced by them); aggregates/math → filter `!g.parentId`. Any new form-open path must go through `goalFormReset()`.
